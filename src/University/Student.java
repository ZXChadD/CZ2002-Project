...
public class Student
{
	...
	public void printTranscript(ArrayList<Course> course, int courseCount, ArrayList<Marks> marks)
	{
		...
		for(int i=0; i<courseCount; i++) {
			for(int j=0; j<course.get(i).lecGrp.get(0).studIds.size();j++) {
				if(studId == course.get(i).lecGrp.get(0).studIds.get(j)) {
			...
			for(int j=0; j<course.get(i).lecGrp.get(1).studIds.size();j++) {
				if(studId == course.get(i).lecGrp.get(1).studIds.get(j)) {
					for(k=0; k<marks.size(); k++) {
						if(course.get(i).getCourseId()==marks.get(k).getCourseId())
							break;
					}
